...
type mlDsaSigGenTestGroup struct {
	...
	ExternalMu         string `json:"externalMu`
	Tests         []struct {
		ID      uint64               `json:"tcId"`
	...
	for _, group := range groups {
		...
		for _, test := range group.Tests {
			results, err := m.Transact("ML-DSA/"+group.ParameterSet+"/sigGen",
			                            1, test.SK, test.Message, test.MU, test.RND, test.Context,[]byte(group.ExternalMu))
			if err != nil {
				return nil, err
			}
	...
	for _, group := range groups {
		...
		for _, test := range group.Tests {
			results, err := m.Transact("ML-DSA/"+group.ParameterSet+"/sigVer", 1,
			                           test.Signature, test.PK, test.Message, test.MU, test.Context, []byte(group.ExternalMu))
			if err != nil {
				return nil, err
			}
